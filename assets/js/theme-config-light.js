...
var themeConfig = {
    ...
    initialize: function () {
        ...
        $('head').append($('<link rel="stylesheet">').attr('href', 'http://localhost/actitud_talento/assets/css/theme-config.css'));
        $this.build();
        $this.events();
    ...
    setColor: function (color) {
        ...
        $colorConfigLink.attr('href', 'http://localhost/actitud_talento/assets/css/theme-' + color + '.css');
        $.cookie('color', color);
    },
    setBackground: function (background) {
        ...
        if (background == 'dark') {
            $('.partners-carousel img').each(function () {
                ...
                $(this).attr('src', 'http://localhost/actitud_talento/assets/img/patrocinadores/' + background + '/' + arr[arr.length - 1]);
            });
        } else {
            $('.partners-carousel img').each(function () {
                var arr = $(this).attr('src').split('/');
                $(this).attr('src', 'http://localhost/actitud_talento/assets/img/patrocinadores/' + arr[arr.length - 1]);
            });
        }
    },
